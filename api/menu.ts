...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const url = `https://dining.utk.edu/wp-admin/admin-ajax.php?action=get_cached_menu&service_area_id=${service_area_id}&date=${date}`;
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to fetch from UTK dining API" });
  }
}